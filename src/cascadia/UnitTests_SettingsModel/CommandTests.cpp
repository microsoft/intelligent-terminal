...
namespace SettingsModelUnitTests
{
    class CommandTests : public JsonTestClass
    {
        ...
        TEST_METHOD(TestDurableAgentMetadata);
    };
    ...
    void CommandTests::TestDurableAgentMetadata()
    {
        NewTerminalArgs args;
        args.AgentSessionId(L"shell-session");
        args.AgentSessionAgent(L"copilot");
        args.AgentPaneAgent(L"claude");
        args.AgentPaneCustomCommand(L"custom-acp --stdio");
        args.AgentPaneView(L"chat");
        args.AgentPaneOpen(true);
        args.AgentPanePosition(L"right");
        args.AgentPaneSize(0.35f);

        auto json = implementation::NewTerminalArgs::ToJson(args);
        VERIFY_IS_TRUE(json["agentSession"].isObject());
        VERIFY_ARE_EQUAL("shell-session", json["agentSession"]["agentSessionId"].asString());
        VERIFY_ARE_EQUAL("copilot", json["agentSession"]["agent"].asString());
        VERIFY_IS_FALSE(json["agentSession"].isMember("paneSessionId"));

        VERIFY_IS_TRUE(json["agentPane"].isObject());
        VERIFY_ARE_EQUAL("claude", json["agentPane"]["agent"].asString());
        VERIFY_ARE_EQUAL("custom-acp --stdio", json["agentPane"]["customCommand"].asString());
        VERIFY_IS_FALSE(json["agentPane"].isMember("agentSessionId"));
        VERIFY_ARE_EQUAL("chat", json["agentPane"]["view"].asString());
        VERIFY_IS_TRUE(json["agentPane"]["open"].asBool());
        VERIFY_ARE_EQUAL("right", json["agentPane"]["position"].asString());
        VERIFY_ARE_EQUAL(0.35f, json["agentPane"]["size"].asFloat());

        const auto parsedWithoutSession = implementation::NewTerminalArgs::FromJson(json);
        VERIFY_ARE_EQUAL(winrt::hstring{ L"claude" }, parsedWithoutSession.AgentPaneAgent());
        VERIFY_ARE_EQUAL(winrt::hstring{ L"custom-acp --stdio" }, parsedWithoutSession.AgentPaneCustomCommand());
        VERIFY_IS_TRUE(parsedWithoutSession.AgentPaneSessionId().empty());
        VERIFY_ARE_EQUAL(0.35f, parsedWithoutSession.AgentPaneSize());

        args.AgentPaneSessionId(L"acp-session");
        json = implementation::NewTerminalArgs::ToJson(args);
        VERIFY_ARE_EQUAL("acp-session", json["agentPane"]["agentSessionId"].asString());

        const auto parsed = implementation::NewTerminalArgs::FromJson(json);
        VERIFY_ARE_EQUAL(winrt::hstring{ L"shell-session" }, parsed.AgentSessionId());
        VERIFY_ARE_EQUAL(winrt::hstring{ L"copilot" }, parsed.AgentSessionAgent());
        VERIFY_ARE_EQUAL(winrt::hstring{ L"acp-session" }, parsed.AgentPaneSessionId());
        VERIFY_ARE_EQUAL(winrt::hstring{ L"claude" }, parsed.AgentPaneAgent());
        VERIFY_ARE_EQUAL(winrt::hstring{ L"right" }, parsed.AgentPanePosition());
    }
}
