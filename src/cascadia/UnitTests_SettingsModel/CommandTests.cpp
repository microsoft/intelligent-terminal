...
namespace SettingsModelUnitTests
{
    class CommandTests : public JsonTestClass
    {
        ...
        TEST_METHOD(ParsePowerShellCompletionItems);

        TEST_METHOD(TestGenerateCommandline);
    };

    void CommandTests::ParsePowerShellCompletionItems()
    {
        const auto items = CompletionItem::ParsePowerShell(
            LR"([{"CompletionText":"Get-ChildItem","ListItemText":"Get-ChildItem","ResultType":2,"ToolTip":"Gets child items.","ApplyMode":"psreadline-v1"}])",
            3,
            4,
            7);
        ...
        const auto item = items.GetAt(0);
        VERIFY_ARE_EQUAL(L"Get-ChildItem", item.CompletionText());
        VERIFY_ARE_EQUAL(L"Get-ChildItem", item.DisplayText());
        VERIFY_ARE_EQUAL(L"Gets child items.", item.Description());
        VERIFY_ARE_EQUAL(2, item.ResultType());
        VERIFY_ARE_EQUAL(L"\ue756", item.IconGlyph());
        VERIFY_ARE_EQUAL(L"psreadline-v1", item.ApplyMode());
        VERIFY_ARE_EQUAL(3u, item.ReplacementIndex());
        VERIFY_ARE_EQUAL(4u, item.ReplacementLength());
        VERIFY_ARE_EQUAL(7u, item.CursorIndex());
    }
