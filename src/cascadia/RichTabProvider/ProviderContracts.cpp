...
namespace Microsoft::Terminal::RichTab::Provider
{
        ...
        const Manifest& manifest)
    {
        ...
        for (const auto& [id, value] : request.firstPartyFields)
        {
            if (!IsCanonicalFieldId(id))
                result.errors.emplace_back("first-party field id is invalid");
            if (value.size() > 4096 || !_IsValidUtf8(value))
                result.errors.emplace_back("first-party field value must be bounded UTF-8");
        }
        ...
        Json::Value firstPartyFields{ Json::objectValue };
        for (const auto& [id, value] : request.firstPartyFields)
        {
