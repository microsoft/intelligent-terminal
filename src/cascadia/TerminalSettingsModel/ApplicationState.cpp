...
#undef COMMA
    ...
    // Method Description:
    // - Save (or overwrite) a durable shell session under `name`. Unlike
    //   workspaces, shell sessions persist after being restored, so the user
    //   can reopen the same saved tab more than once.
    void ApplicationState::SaveShellSession(const hstring& name, const Model::WindowLayout& layout)
    {
        {
            const auto state = _state.lock();
            if (!state->ShellSessions || !*state->ShellSessions)
            {
                state->ShellSessions = winrt::single_threaded_map<hstring, Model::WindowLayout>();
            }
            (*state->ShellSessions).Insert(name, layout);
        }
        _throttler();
    }

    // Method Description:
    // - Remove the durable shell session stored under `name`.
    // Return Value:
    // - true if an entry was removed, false otherwise.
    bool ApplicationState::RemoveShellSession(const hstring& name)
    {
        bool removed{ false };
        {
            const auto state = _state.lock();
    ...
    // Method Description:
    // - Look up a durable shell session by `name` WITHOUT removing it (the
    //   restore path is non-destructive so a session can be reopened again).
    // Return Value:
    // - The stored layout, or nullptr if there was none.
    Model::WindowLayout ApplicationState::GetShellSession(const hstring& name)
    {
        const auto state = _state.lock_shared();
        if (state->ShellSessions && *state->ShellSessions)
        {
    ...
    Windows::Foundation::Collections::IMapView<hstring, Model::WindowLayout> ApplicationState::AllShellSessions()
    {
        const auto state = _state.lock_shared();
        if (state->ShellSessions && *state->ShellSessions)
        {
            return (*state->ShellSessions).GetView();
        }
        return nullptr;
    }
