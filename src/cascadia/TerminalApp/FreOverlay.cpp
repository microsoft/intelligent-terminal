...
namespace winrt::TerminalApp::implementation
{
    namespace
    {
        ...
        // WslDistroGenerator. We prefer the `-d <distName>` token in
        ...
        inline std::wstring _ExtractWslDistroName(const winrt::Microsoft::Terminal::Settings::Model::Profile& profile)
        {
            const std::wstring cmd{ profile.Commandline() };
            // Look for ` -d <token>` or ` --distribution <token>`.
            // Whitespace-bounded so we don't match `--default-d` etc.
        ...
        //
        // MUST be called on the UI thread — _settings.AllProfiles()
        // returns the live observable vector, and iterating it
        // concurrently with a settings reload (file watcher or user
        // save on the UI thread) would be a use-after-mutate. We
        // copy out plain strings so callers can move to a background
        // thread to actually run the install / uninstall.
        inline std::vector<std::wstring> _SnapshotWslDistroNames(
        ...
        // profile for. Same UI-thread restriction as
        // _SnapshotWslDistroNames — _settings.AllProfiles() is an
        // observable live vector. Used to gate Install so we don't
        // pollute HOME with a block for a shell the user has no
        // profile for.
        ...
        inline ShellPresence _SnapshotShellPresence(
            const winrt::Microsoft::Terminal::Settings::Model::CascadiaSettings& settings)
        {
            ShellPresence out{};
            if (!settings)
    ...
    IAsyncAction FreOverlay::_SaveAndInstallAsync()
    {
        ...
        if (AutoDetectToggle().IsOn())
        {
            ...
            // A skipped shell is reported as success-already-installed
            // so the FRE failure verdict (below) doesn't flag a
            // missing shell as an integration failure.
            const auto skipped = []() {
                SI::InstallResult r{};
                r.success = true;
            ...
            const auto pwsh7Result = shellPresence.pwsh
                                         ? SI::InstallForTarget(SI::Target::Pwsh)
                                         : skipped();
            const auto windowsPsResult = shellPresence.windowsPowerShell
                                             ? SI::InstallForTarget(SI::Target::WindowsPowerShell)
                                             : skipped();
            const auto bashResult = shellPresence.bash
                                        ? SI::InstallForTarget(SI::Target::Bash)
                                        : skipped();

            // WSL distros — one install per distro the user has a WT
            // profile for (covered by the snapshot above).
            std::vector<std::pair<std::wstring, SI::InstallResult>> wslResults;
            for (const auto& distName : wslDistros)
            {
                wslResults.emplace_back(distName, SI::InstallWslBash(distName));
            }
