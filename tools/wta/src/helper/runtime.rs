...
async fn run_acp_app(
    ...
    local_set
        .run_until(async move {
            ...
            if is_host_agent_source {
                match cli.custom_models.as_deref() {
                    Some(custom_models) => match serde_json::from_str(custom_models) {
                        Ok(models) => {
                            app_state.set_custom_model_config(models, cli.custom_model_selection.clone())
                        }
            ...
            app_state.set_host_catalog_ready(!is_host_agent_source || cli.custom_models.is_some());
