...
fn main() {
    ...
    //
    // Why this is needed:
    //   Large unoptimized application/event-dispatch functions can exceed
    //   the default 1 MB main-thread stack. Localization initialization is
    //   separately sharded below so it also fits ordinary worker/test stacks.
    //
    ...
    generate_locale_backends(&manifest_dir, &out_dir);
...
/// rust-i18n 3 emits one HashMap temporary per translation into one initializer.
/// Splitting by locale bounds its unoptimized stack frame as languages are added.
fn generate_locale_backends(manifest_dir: &Path, out_dir: &Path) {
    let locales_dir = manifest_dir.join("locales");
    println!("cargo:rerun-if-changed={}", locales_dir.display());
    let mut locales: Vec<_> = fs::read_dir(&locales_dir)
        .expect("read locale directory")
        .map(|entry| entry.expect("read locale entry").path())
        .filter(|path| path.extension().is_some_and(|ext| ext == "yml"))
        .collect();
    locales.sort();

    let shards_dir = out_dir.join("locale_shards");
    let empty_dir = shards_dir.join("empty");
    fs::create_dir_all(&empty_dir).expect("create empty root locale directory");
    let mut generated = String::from(
        "// Generated by build.rs. Do not edit.\n\
         i18n!(",
    );
    generated.push_str(&format!(
        "{:?}, fallback = \"en-US\", backend = ShardedLocaleBackend);\n\
         struct ShardedLocaleBackend;\n",
        empty_dir.to_str().expect("UTF-8 build output path"),
    ));
    let mut available = String::new();
    let mut translate = String::new();
    for (index, source) in locales.iter().enumerate() {
        let locale = source
            .file_stem()
            .and_then(|stem| stem.to_str())
            .expect("UTF-8 locale filename");
        let shard_dir = shards_dir.join(locale);
        fs::create_dir_all(&shard_dir).expect("create locale shard directory");
        fs::copy(
            source,
            shard_dir.join(source.file_name().expect("locale filename")),
        )
        .expect("copy locale into shard");
        println!("cargo:rerun-if-changed={}", source.display());
        generated.push_str(&format!(
            "#[allow(unused_imports)]\n\
             mod locale_shard_{index} {{\n\
                 i18n!({path:?});\n\
                 pub(super) fn translate(locale: &str, key: &str) -> Option<&'static str> {{\n\
                     _RUST_I18N_BACKEND.translate(locale, key)\n\
                 }}\n\
             }}\n",
            path = shard_dir.to_str().expect("UTF-8 locale shard path"),
        ));
        available.push_str(&format!("{locale:?},"));
        translate.push_str(&format!(
            "{locale:?} => locale_shard_{index}::translate(locale, key),\n",
        ));
    }
    generated.push_str(&format!(
        "impl rust_i18n::Backend for ShardedLocaleBackend {{\n\
             fn available_locales(&self) -> Vec<&str> {{ vec![{available}] }}\n\
             fn translate(&self, locale: &str, key: &str) -> Option<&str> {{\n\
                 match locale {{ {translate} _ => None }}\n\
             }}\n\
         }}\n",
    ));
    fs::write(out_dir.join("locale_codegen.rs"), generated).expect("write locale backend code");
}
