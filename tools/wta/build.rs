...
//
// Build-time codegen for the WTA ETW TraceLogging provider and locale names.
//
...
fn main() {
    ...
    generate_locale_names(&manifest_dir.join("locales"), &out_dir);
...
fn generate_locale_names(locales_dir: &Path, out_dir: &Path) {
    let mut locales = BTreeSet::new();
    collect_locale_names(locales_dir, &mut locales);
    assert!(locales.contains("en-US"), "missing fallback locale en-US");

    // rust-i18n 3.x returns sorted, deduplicated names. That order also decides
    // normalize_locale's language-prefix fallback.
    let names = locales
        .iter()
        .map(|locale| format!("    {locale:?},\n"))
        .collect::<String>();
    let out_path = out_dir.join("locale_names_codegen.rs");
    fs::write(
        &out_path,
        format!(
            "// Generated by build.rs; do not edit.\n\
             const AVAILABLE_LOCALE_NAMES: &[&str] = &[\n{names}];\n"
        ),
    )
    .unwrap_or_else(|e| panic!("failed to write {}: {e}", out_path.display()));
}

fn collect_locale_names(dir: &Path, locales: &mut BTreeSet<String>) {
    // Directory tracking also rebuilds the catalog when files are added/removed.
    ...
    for entry in entries {
        let path = entry
            ...
            .path();
        if path.is_dir() {
            collect_locale_names(&path, locales);
            continue;
        }
        let Some(extension) = path.extension().and_then(|s| s.to_str()) else {
            continue;
        };
        if !matches!(extension, "yml" | "yaml" | "json" | "toml") {
            continue;
        }

        // The catalog uses flat, dotted-key YAML (rust-i18n's v1 format).
        // Fail closed if that changes: v2 takes locale names from values,
        // not filenames, and would require a different enumeration strategy.
        assert!(
            matches!(extension, "yml" | "yaml"),
            "locale name generation requires flat YAML: {}",
            path.display()
        );
        ...
        for line in content.lines().map(str::trim) {
            ...
            assert!(
                ...
                "locale name generation requires flat dotted-key YAML: {}",
                path.display()
            );
            has_translation = true;
        }
        assert!(has_translation, "empty locale catalog: {}", path.display());

        // Match rust-i18n-support's filename handling (e.g. ui.en-US.yml).
        let locale = path
            .file_stem()
            .and_then(|s| s.to_str())
            .and_then(|s| s.rsplit('.').next())
            .unwrap_or_else(|| panic!("invalid locale filename: {}", path.display()));
