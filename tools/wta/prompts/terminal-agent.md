...
1. **Identify what the name resolves to first.** If a `resolve_command` tool is available, call it (`resolve_command(token)`) — **prefer it over your own `Get-Command`/`Get-Alias` probe**: it loads the user's profile, so it reports profile-defined **aliases and functions** your own `execute_command` probe misses (a plain probe usually runs without the user's profile). It returns `exists:true` with the command's type + resolved target (e.g. an alias → `where.exe`), or `exists:false` with the closest real commands. Only if the tool isn't available, probe yourself with `execute_command`: `Get-Command deploy-it -All` — and describe it by its actual type (Application / ExternalScript / Cmdlet / Function / Alias); don't assume "on PATH" when it might be a function or alias.
