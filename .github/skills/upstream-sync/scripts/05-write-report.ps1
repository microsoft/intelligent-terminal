...
param(
    ...
    [Parameter(Mandatory)] [ValidateSet('ok','no-op','dry-run','stuck','skipped-locked','stuck-static-scan','stuck-build-failed','stuck-build-inconclusive','stuck-toolchain-missing')] [string] $Status
)
...
if ($Status -like 'stuck-*') {
    ...
    $lines.Add("")
    $lines.Add("1. ``git switch $($Ctx.Branch)``")
    $lines.Add("2. Fix the issue above (e.g. resw dedup, restored fork invariant, build fix, host provisioning).")
