...
| Diagnostic logs | Sensitive | Known examples under the per-version log dir `…\LocalCache\Local\IntelligentTerminal\logs\<pkgver>\` include `wta-main_*.log`, `wta-delegate.log`, `terminal-agent-pane.log`, `rich-tabs-<pid>-<epoch>.jsonl`, `wta-install-hooks.log`, and `hook-trace.log`. Raw user/agent content is logged at `trace` only; Rich Tab logs never contain raw CWD, provider/tab/session identity, snapshot/presentation content, leases, provider output, environment, or external error text. Rich Tab files cap each process at 8 MiB and prune closed runs after 7 days toward a 32 MiB budget. |
...
| Diagnostic logs may disclose sensitive data | Information disclosure | Medium | WTA and hook logs may contain command lines, event payload summaries, errors, and metadata. Raw user/agent content is gated to `trace`; Rich Tab JSONL uses an event-specific scalar whitelist and process-local aliases, excluding raw paths, provider output, snapshots, presentation text, leases, environment, and external error strings at every level. Known examples include `wta-main_*.log`, `wta-delegate.log`, `terminal-agent-pane.log`, `rich-tabs-*.jsonl`, `wta-install-hooks.log`, and `hook-trace.log` under `logs\<pkgver>\`. Retention and file sizes are bounded per writer. |
