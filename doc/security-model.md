...
| **C-HOOK** | Agent CLI hook bridge -> `wtcli` / COM -> WTA subscribers | Third-party CLI hook system launches `wtcli agent-hook` directly, which reads stdin and calls COM `SendEvent`; WTA receives events through `wtcli --json listen` / COM callbacks | Plugin-manager installation plus observed COM activation behavior. `WT_COM_HOOK_CLSID` is non-secret process-lifetime routing metadata: expiry prevents late hooks from restarting Terminal, not unauthorized access to a live server. Hook payloads are untrusted and legacy `agent_event` broadcasts are not source-bound or subscriber-filtered today. This channel does not carry `send_input`. |
...
| Diagnostic logs | Sensitive | Known examples under the per-version log dir `…\LocalCache\Local\IntelligentTerminal\logs\<pkgver>\` include `wta-main_*.log`, `wta-delegate.<UTC-date>.log` or fallback `wta-delegate.log`, `terminal-agent-pane.log`, and `wta-install-hooks.<UTC-date>.log` or fallback `wta-install-hooks.log`. At the helper's WT event receipt boundary, Release builds emit one `info` record with the method only; internal Debug builds instead emit one `debug` record with the method and full envelope. Separate routing diagnostics may still record metadata at `trace`. Other raw user/agent content (prompts, responses, terminal output, typed input) remains gated to `trace`. Retention: only the current version's log dir is kept, UTC-dated Rust WTA streams retain up to three files, and per-PID helper logs prune after 3 days. |
...
| Diagnostic logs may disclose sensitive data | Information disclosure | Medium | WTA logs are not redacted and may contain command lines, event payload summaries, errors, and metadata. At the helper's WT event receipt boundary, Release builds emit one `info` record with the method only; internal Debug builds instead emit one `debug` record with the method and full envelope. Separate routing diagnostics may still record metadata at `trace`. Other raw user/agent content (prompts, responses, terminal output, typed input) remains gated to `trace`. Known examples include `wta-main_*.log`, `wta-delegate.<UTC-date>.log` or fallback `wta-delegate.log`, `terminal-agent-pane.log`, and `wta-install-hooks.<UTC-date>.log` or fallback `wta-install-hooks.log` under `logs\<pkgver>\`. Retention is bounded (only the current version's dir kept, UTC-dated Rust WTA streams retain up to three files, per-PID helper logs prune after 3 days). |
