...
| **wt-agent-hooks bridge** | Agent CLI plugin/extension invoking `agent-hook.cmd` and `wtcli agent-hook` | Installed into supported Agent CLIs through their own plugin or extension managers. The launcher enforces a fast environment gate and the native command wraps hook JSON and publishes it through COM; it is an event/status bridge, not a shell-input capability. |
...
| **C-HOOK** | Agent CLI hook bridge -> `wtcli` / COM -> WTA subscribers | Third-party CLI hook system launches the lightweight `agent-hook.cmd` gate and then `wtcli agent-hook`, which reads stdin and calls COM `SendEvent`; WTA receives events through `wtcli --json listen` / COM callbacks | Plugin-manager installation plus observed COM activation behavior. Hook payloads are untrusted and legacy `agent_event` broadcasts are not source-bound or subscriber-filtered today. This channel does not carry `send_input`. |
...
flowchart TB
    ...
    subgraph AgentZone["External Agent CLI"]
        ...
        HookBridge(["agent-hook.cmd<br/>wtcli agent-hook bridge"])
    end
...
Supported Agent CLI hook fires in a pane, if hooks are installed
  -> agent-hook.cmd validates WT_COM_CLSID + WT_SESSION
