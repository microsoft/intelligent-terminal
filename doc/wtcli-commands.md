...
| `list-shell-sessions` | — | List saved durable shell sessions for the caller's elevation scope, annotated with whether each one is already open in a tab. | `wtcli --json list-shell-sessions` | ✅ `cli_channel.rs` (`list_shell_sessions`) |
| `restore-shell-session` | — | Restore a saved shell session into a window, or focus it if a tab is already bound to that id. `-w` selects the target window and defaults to the most recent one. | `wtcli restore-shell-session 12 -w 1` | ✅ `cli_channel.rs` (`restore_shell_session`) |
