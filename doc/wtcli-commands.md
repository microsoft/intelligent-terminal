...
| `list-tab-sessions` | — | List saved durable tab sessions for the caller's elevation scope, annotated with whether each one is already open in a tab. | `wtcli --json list-tab-sessions` | ✅ `cli_channel.rs` (`list_durable_tab_sessions`) |
| `restore-tab-session` | — | Restore a saved tab session into a window, or focus it if a tab is already bound to that id. `-w` selects the target window and defaults to the most recent one. | `wtcli restore-tab-session 12 -w 1` | ✅ `cli_channel.rs` (`restore_durable_tab_session`) |
