...
- **状态**：分步实现中；TDD Step 0-7 已完成，outer containment 与 final integration 待完成
- **首次调查**：2026-07-17
- **最后核验**：2026-07-22
...
| Usage receive | master 已可靠 coalesce/定向 latest value；helper 已通过 provider-neutral normalizer fail-fast校验 typed `UsageUpdate`，按session lifecycle保存并通过现有state projection发送 | Step 8增加一次outer containment与usage-specific redaction |
...
| C++ event route | 现有`agent_state_changed`按`tab_id`路由并消费可选`usage`/null；missing保持、null清除、malformed fail-fast；`_UpdateBottomBarState`从active tab cache渲染 | 不新增COM/IDL route；异常降级只放在Step 8最外层Usage boundary |
