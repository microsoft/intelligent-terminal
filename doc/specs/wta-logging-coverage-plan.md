...
`prune_old_version_dirs` **只保留当前版本目录,其余版本目录每次启动整目录删除**。
...
拼写:app.rs 注释里那个不在词典的英文词改成 `can be reconstructed`(过 check-spelling)。
...
- 在 `logs\` 下手动造几个旧版本子目录后重启,只保留当前版本目录,其余版本目录整个被删,
  当前版本目录不受影响(改动 H/I 用按版本分目录取代了早期的 `.wta-log-version` 标记方案)。
