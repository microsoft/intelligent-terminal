...
- [ ] `C312` `[new]` `[UT✓]` `[E2E]` **Queued user messages run once in submission order:** Messages submitted during an active response receive an Info notice and stay pinned above the input while chat actually scrolls; each reaches the agent exactly once after its independently held predecessor finishes. Immediately dispatched messages do not show a queued notice. _(UT: `fifo_keeps_active_stream_contiguous_and_drains_only_at_terminal_boundary`, `waiting_messages_receive_one_info_notice_each_but_immediate_sends_do_not`, `render_prompt_queue_stays_pinned_while_chat_scrolls`; E2E: `Feature.PromptQueue`.)_
...
- [x] `C076` `[UT✓]` **`/fix` works:** Runs manual autofix using recent terminal context, queueing behind an active turn without replacing it. _(UT: classify + `slash_fix_when_idle_submits_autofix_turn` / `slash_fix_while_busy_queues_without_replacing_active_turn`.)_
- [ ] `C313` `[new]` `[E2E]` **Typed fix preserves captured evidence while waiting:** A typed `/fix` submitted while the agent is busy captures the current failed shell context before dispatch. After capture completes, a later shell command neither discards the explicit request nor substitutes newer output or its working directory. _(E2E: `Feature.PromptQueue`.)_
...
- [ ] `C314` `[new]` `[E2E]` **Pending queue appears automatically and updates above input:** Pending requests automatically show `Queued messages: N` (en-US) and numbered, sanitized previews above input, including labelled automatic fixes, `Alt+R Recall last` for user requests, and `Alt+D Discard remaining`. Only paused queues show Send remaining; empty queues have no buttons. Status and controls take priority over previews in short panes, with compact shortcut labels when needed. The list stays outside chat scrolling and updates as requests dispatch or are removed. Displaying the list never sends an agent prompt. _(E2E: `Feature.PromptQueue`.)_
- [ ] `C315` `[new]` `[UT✓]` `[E2E]` **Stopping a turn keeps explicit requests paused until Send remaining:** `/stop` or unselected physical `Ctrl+C` cancels active work, discards automatic requests, and retains unsent explicit requests under `Paused: N pending; not sent automatically` (en-US). New input joins the stopped queue without resuming it. Neither an early resume attempt nor cancellation settlement sends anything; after settlement, physical `Alt+S` or the rendered Send remaining button sends retained requests once in FIFO order without replaying the cancelled turn. _(E2E: `Feature.PromptQueue`; unit tests cover additional connection, permission, and configuration barriers.)_
- [ ] `C316` `[new]` `[UT✓]` `[E2E]` **Failed turns keep explicit requests paused until explicitly discarded:** A failed ACP prompt discards automatic requests but keeps unsent explicit requests stopped, including newly entered messages, without replaying the failed turn. Physical `Alt+D` clears pending work without clearing the draft; submitting that draft afterward starts fresh work normally. Soft-stop follows the same pause contract (unit coverage). _(E2E: `Feature.PromptQueue`.)_
- [ ] `C317` `[new]` `[E2E]` **Recall restores the last pending request without duplicate submission:** Physical `Alt+R` restores the newest pending explicit request, including its image attachment, only into an empty editor and removes its original queue entry. An occupied draft rejects recall without changing either request. Re-entering the edited request sends it exactly once, after the older pending request, with its image intact. _(E2E: `Feature.PromptQueue`.)_
- [ ] `C318` `[new]` `[E2E]` **Discard remaining preserves the draft and never cancels active work:** Clicking Discard remaining clears a stopped queue while preserving the editor and the existing cancellation barrier. Fresh input subsequently works. Physical `Alt+D` can also discard pending requests behind a running turn without cancelling that turn, submitting the draft, or clearing the editor. _(E2E: `Feature.PromptQueue`.)_
...
- [ ] `C095` `[UT✓]` `[E2E]` **Autofix waits for agent startup without losing the failure:** A shell failure received by a running helper while the agent starts is captured and submitted exactly once when ACP is ready. Events missed before the helper subscribed are not recovered, and successful commands do not enqueue Autofix. _(UT: `cold_start_captures_autofix_without_dispatch_before_connected`, `startup_queue_waits_for_connected_and_configuration`; E2E: `Feature.PromptQueue`.)_
- [ ] `C311` `[new]` `[UT✓]` `[E2E]` **Autofix detection stays actionable while the agent connects:** With automatic suggestion off, an error still shows an enabled diagnostics button before ACP is ready. Clicking it queues the fix without starting analysis early; successful commands and disabled detection do not show diagnostics. _(UT: `DetectionVisibilityDoesNotRequireConnectionState`, `DisabledDetectionHidesAllDiagnosticStates`, `detected_autofix_is_actionable_while_connecting_and_queues_until_ready`; E2E: `Feature.PromptQueue`.)_
- [ ] `C307` `[new]` `[E2E]` **Repeated diagnostics activation submits one fix per failure:** Repeated clicks on the same detected error while ACP session creation is held leave only one waiting request and run it once on connection. A fresh later failure remains separately actionable and still requires a click when automatic suggestions are off. _(E2E: `Feature.PromptQueue`.)_
- [ ] `C308` `[new]` `[UT✓]` `[E2E]` **Queued Autofix stays pinned to its failed source pane:** When Autofix waits behind another request, moving focus to a sibling pane does not replace the captured failure evidence or its source. _(UT: `autofix_snapshot_freezes_evidence_and_target_without_dispatch_reads`, `rename_updates_routing_but_never_changes_pinned_source`; E2E: `Feature.PromptQueue`.)_
- [ ] `C310` `[new]` `[UT✓]` `[E2E]` **New shell commands invalidate obsolete queued Autofix:** Starting new work in the failed source pane cancels its obsolete automatic request before dispatch, while a later fresh failure remains eligible. Normal prompt redraw after a failure does not discard the queued request. _(UT: `per_pane_echoes_and_prompt_end_preserve_pending_failures`, `shell_prompt_end_preserves_active_autofix_but_command_start_cancels_it`; E2E: `Feature.PromptQueue`.)_
- [ ] `C309` `[new]` `[E2E]` **Prompt redraws preserve queued Autofix until a real command starts:** Repeated source-pane OSC 133 A/B prompt redraws without C preserve the queued failure and deliver its captured evidence exactly once. Genuine new shell work still invalidates obsolete automatic requests. _(E2E: `Feature.PromptQueue`; PSReadLine prompt redraws cross the native VT and helper boundaries.)_
