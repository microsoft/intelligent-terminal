...
- [ ] `C264` `[new]` `[UT✓]` `[E2E]` **Completed turns toggle and select from rendered prompt targets:** A left-button Down/Up on the same visible turn's `▶` / `▼` or actual rendered user-input cells collapses or expands only that turn and reuses the keyboard selection, highlight, navigation, Esc, and Enter behavior. Prefixes, row-end whitespace, details, drags, overlays, tab changes, scrolling, and stale/offscreen coordinates do not toggle; multiline, wrapped, wide-cell, and double/triple-click paths preserve their expected geometry and text-selection behavior. _(UT: `clicking_multiline_completed_turn_prompt_selects_and_reuses_enter_toggle`, `completed_turn_user_input_hits_follow_rendered_text_boundaries`, `completed_turn_user_input_multiclick_preserves_turn_state_and_text_selection`, `completed_turn_mouse_selection_continues_with_keyboard_navigation`, `completed_turn_triangle_click_ignores_text_drag_and_hidden_chat`, `completed_turn_triangle_hits_follow_visible_scrolled_turns`; E2E: `Feature.AgentMouse` `CompletedTurnMouse` / `CompletedTurnPromptMouse`.)_
...
- [ ] `C259` `[new]` `[UT✓]` `[E2E]` **BYOM provider serves the selected model end to end:** A keyless local OpenAI-compatible provider selected in Settings is isolated into Copilot, receives chat-completions requests for the configured model, and remains the only `/model` choice. _(UT: custom-provider environment and settings launch-configuration tests; #447; E2E: `Feature.ByomProvider`.)_
- [ ] `C260` `[new]` `[UT✓]` `[E2E]` **Leaving BYOM restarts and restores cloud models:** Clearing the custom model selection rebuilds the shared agent stack, reconnects Copilot without provider overrides, and restores its native cloud catalog. _(UT: clean cloud discovery and settings rebuild tests; #447; E2E: `Feature.ByomProvider`.)_
