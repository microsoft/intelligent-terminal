...
- [x] `[UT✓]` **Idle state is correct:** A live session waiting for the next prompt shows idle/ready state. _(UT: Idle activity derivation `agent_sessions.rs::activity_and_liveness_derive_from_legacy_status` + badge render `agents_view.rs::status_badge_renders_expected_text_per_state`; live-picker E2E infeasible — needs a live shell session whose pane contends with the SessionToggleButton toggle-race.)_
- [x] `[UT✓]` **Ended state is correct:** A session whose pane was closed becomes ended and does not stay falsely live. _(UT: PaneClosed tombstone + "Ended must stay Ended" in `agent_sessions.rs`; Ended renders an empty badge `agents_view.rs::status_badge_renders_expected_text_per_state`, so it is visually distinct from any live/idle row.)_
