...
- [x] `C129` `[UT✓]` **Modifiers do not change Enter behavior:** Enter is the only activation gesture — Shift+Enter resumes/focuses exactly like Enter, and there is no alternate resume path. _(UT: `shift_enter_on_class_a_dead_row_ignores_shift`, `shift_enter_on_class_b_dead_row_ignores_shift`, `shift_enter_on_class_a_live_row_focuses`. Not E2E: Live-row Enter dispatches FocusPane — moves WT focus to the session's pane, it does NOT dismiss the view — and the MVP picker shows only Class B shell sessions whose panes are usually already closed, so there is no stable E2E observable.)_
