...
- [ ] `C278` `[new]` `[E2E]` **Legacy hook bundle without WT_SESSION stays unattributed:** A pre-#571 hook whose process never inherited `WT_SESSION` still publishes, but with an empty `pane_id` instead of the focused pane's — so it cannot evict the focused pane's real session or misdirect Enter in the session list. _(#578; `wtcli send-event` no longer falls back to `GetActivePane()`.)_
