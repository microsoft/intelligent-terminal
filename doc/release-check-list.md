...
- [ ] `C314` `[new]` `[E2E]` **Pending queue appears automatically and updates above input:** Pending requests automatically show a count header and numbered, sanitized previews above input, including labelled automatic fixes. The list stays outside chat scrolling, updates as requests dispatch or cancel, and disappears when empty; previews and overflow respect available space. Displaying the list never sends an agent prompt. _(E2E: `Feature.PromptQueue`.)_
- [ ] `C315` `[new]` `[UT✓]` `[E2E]` **Stopping a turn cancels queued messages and accepts new input:** `/stop` cancels waiting messages with an Info notice, preserves the active cancellation barrier, and allows fresh input after settlement without a resume command. _(UT: `stop_discards_pending_requests_and_future_input_works_without_recovery_command`, `queue_preserves_new_input_when_an_earlier_cancellation_settles_as_an_error`, `ctrl_c_cancels_waiting_input_without_arming_pane_close`; E2E: `Feature.PromptQueue`.)_
- [ ] `C316` `[new]` `[UT✓]` `[E2E]` **Failed turns discard queued messages and accept new input:** A failed ACP prompt cancels waiting requests with an Info notice, never replays failed or discarded work, and leaves new input usable. _(UT: `errors_and_soft_stops_discard_pending_without_replaying_active_work`; E2E: `Feature.PromptQueue`.)_
