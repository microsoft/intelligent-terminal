...
- [ ] `C318` `[new]` `[E2E]` **Saved agent sessions resume in new tabs without prewarm replacement:** An accepted resume request loads the exact saved ACP session into the new tab's helper, restores its transcript without a fresh session or prompt, and remains correct for a second independent session. Ordinary tabs still prewarm once, and resumed tabs preserve their session when hidden and restored. _(PR #938; E2E: `Feature.AgentPaneLifetime`, tag `SessionResume`; deterministic persisted ACP fixture.)_
