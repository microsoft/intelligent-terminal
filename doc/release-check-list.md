...
- [ ] `C264` `[new]` `[UT~]` `[E2E]` **Bundled hook runs inside its agent CLI:** A real agent CLI session executes the shipped hook command through its own shell and its events reach Terminal. _(#571; UT: `bundled_hook_commands_execute_in_their_cli_shell` runs each bundle's command line in that CLI's shell; E2E adds the CLI accepting the bundle.)_
