...
- [ ] `C284` `[new]` `[UT✓]` `[E2E]` **New-tab command workspaces accept a split-direction hint across Session MCP:** A validated `run_command_in_workspace` request may retain a `split_direction` hint when its placement is `new_tab`. _(#599; E2E: `Feature.AgentProtocolExperience`.)_
