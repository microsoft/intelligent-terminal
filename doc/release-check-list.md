...
- [ ] `C287` `[new]` `[UT✓]` `[E2E]` **One hook broadcast applies once per helper fan-out:** One `wtcli agent-hook` invocation reaches every connected helper over the COM broadcast, and each forwards it, but master applies it exactly once — and the synthetic start an unseen session expands into dedupes independently of the reported event. _(#761; UT: `replayed_broadcast_id_is_applied_once_across_sibling_helpers` + `sibling_events_from_one_broadcast_are_not_deduped_against_each_other` + `hook_slots_are_stable_regardless_of_local_session_knowledge`; E2E adds the real multi-helper fan-out those unit tests cannot span.)_
- [ ] `C288` `[new]` `[UT✓]` `[E2E]` **Terminal hook for an unknown session creates no row:** An `agent.session.end` for a session WTA never saw start leaves no session row, so no permanently unprunable `Ended` entry titled after the cwd basename appears in the picker. _(#761; UT: `terminal_agent_event_for_unknown_session_does_not_fabricate_a_row`.)_
- [ ] `C289` `[new]` `[UT✓]` `[E2E]` **Agent error for an unknown session still records the failure:** `agent.error` reports a live but failing session, so it must keep creating the row its pane-keyed reducer needs — excluding it with the terminal events would silently drop a first-observed connection failure. _(#761; UT: `agent_error_for_unknown_session_still_records_the_failure`.)_
