...
- [ ] `C326` `[new]` `[E2E]` **Native tmux tab order follows remote window indices:** Native tab order follows current `window_index`, not permanent `window_id`, on attach and after swaps, moves, renumbering, insertion/removal, and reattachment. Index-only changes preserve tab/pane identity and backend selection. _(E2E: `Feature.TmuxReconnect`.)_
- [ ] `C320` `[new]` `[E2E]` **Default tmux session menu opens an independent window:** The workspace-style menu lists only the SSH host's default server, refreshes on opening, and attaches a session that is not already open in a new native window without changing the source session. Stable IDs survive session renames; opaque backend launches remain compatible. _(#966; E2E: `Feature.TmuxSessionBrowser`.)_
...
- [ ] `C324` `[new]` `[E2E]` **Repeated tmux session menu selections focus the existing window:** Re-selecting an attached session from either menu focuses its existing native window and keeps one backend connection. Stable IDs survive renames; a closed attachment can be opened again. _(E2E: `Feature.TmuxSessionBrowser`.)_
- [ ] `C325` `[new]` `[E2E]` **Native tmux tab rename persists across clients and reattachment:** The existing rename editor changes the targeted backend window, preserves Unicode and literal shell/format characters, and updates both native clients. Cancel leaves the name unchanged; reattach retains it; clearing the custom title restores automatic naming without renaming sibling windows. _(E2E: `Feature.TmuxSessionBrowser`.)_
- [ ] `C322` `[new]` `[E2E]` **Closing a native tmux window preserves remote activity and rebinds on attach:** Closing only the native window detaches without ending remote work or changing its last-known status/tool/error/title/timestamp. The next real packaged v2 hook after reattachment refreshes the same raw SSH session ID with a new native binding, no Host duplicate, and the original remote pane process. Activity while no native receiver exists is explicitly last-known. _(E2E: `Feature.TmuxRemoteHooks`.)_
- [ ] `C323` `[new]` `[E2E]` **Observed remote tmux exit ends the shared SSH session:** Destroying an owned remote session while its native control client is attached emits closed, marks its shared SSH row Ended, and removes the pane binding. A sentinel keeps the server alive so original-socket pane inventory must distinguish true destruction from the same bare `%exit` used by detach. _(E2E: `Feature.TmuxRemoteHooks`.)_
