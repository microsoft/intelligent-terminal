...
- [ ] `C214` `[new]` `[UT~]` `[E2E]` **FRE execution-policy remediation is correct:** FRE automatically changes a genuinely blocking CurrentUser PowerShell execution policy to `RemoteSigned`, verifies the result, and continues setup; an unknown/unreadable policy without a confirmed block remains non-blocking. _(#336/#338/#309; UT: execution-policy gate and remediation helpers.)_
...
- [ ] `C324` `[new]` `[E2E]` **Agent draft undo and redo groups contiguous typing:** Physical Ctrl+Z removes one contiguous typing group without removing an earlier pasted prefix; Ctrl+Y restores that exact group without submitting. _(E2E: `Feature.AgentInputUndoRedo`.)_
- [ ] `C319` `[new]` `[E2E]` **Agent draft undo and redo restores atomic editing transactions:** Selected-draft typing, paste, cut, Backspace and Delete each undo/redo atomically with source and whole-draft selection restored; idle Esc/Ctrl+C clearing is also undoable. _(E2E: `Feature.AgentInputUndoRedo`.)_
- [ ] `C320` `[new]` `[E2E]` **Agent draft undo and redo preserves multiline Unicode source:** Multiline Unicode paste is one transaction, and subsequent caret edits round-trip exact logical newlines and non-ASCII source without corrupting cursor position. _(E2E: `Feature.AgentInputUndoRedo`.)_
- [ ] `C321` `[new]` `[E2E]` **Agent draft edits invalidate redo but copy and caret movement do not:** Copy, selection and caret movement split typing groups without becoming edits or discarding redo; a new edit after undo replaces the redo branch. _(E2E: `Feature.AgentInputUndoRedo`.)_
- [ ] `C322` `[new]` `[E2E]` **Agent draft undo respects submission and prompt history boundaries:** Physical submission clears draft undo history without replaying ACP actions; prompt recall restores the original draft's undo/redo chain, while editing a recalled prompt starts a fresh chain rooted at that prompt. _(E2E: `Feature.AgentInputUndoRedo`.)_
- [ ] `C323` `[new]` `[E2E]` **Agent draft image undo and redo restore the pasted payload:** Real clipboard image-file paste through Alt+V supports insertion and selected-draft replacement undo/redo; a deterministic ACP fixture receives the restored attachment with its original MIME type, byte length and SHA-256. _(E2E: `Feature.AgentInputUndoRedo`.)_
- [ ] `C325` `[new]` `[E2E]` **Mouse clicks reposition the agent draft caret:** Clicking an earlier visible text cell makes subsequent physical typing insert at that source position. _(E2E: `Feature.AgentInputMouseCursor`.)_
- [ ] `C326` `[new]` `[E2E]` **Mouse clicks target visible multiline draft rows:** A click maps the scrolled input viewport to the visible logical line without changing newline structure. _(E2E: `Feature.AgentInputMouseCursor`.)_
- [ ] `C327` `[new]` `[E2E]` **Mouse clicks follow soft-wrapped draft text:** Physical clicks use the rendered wrapped row rather than the beginning or end of the source draft. _(E2E: `Feature.AgentInputMouseCursor`.)_
- [ ] `C328` `[new]` `[E2E]` **Mouse clicks preserve Unicode text and collapse draft selection:** Clicking within a wide character targets its valid source boundary and replaces full-draft selection with an insertion caret. _(E2E: `Feature.AgentInputMouseCursor`.)_
...
- [ ] `C292` `[new]` `[UT✓]` `[E2E]` **AllowAutomaticApproval hides automatic approval and turns it off:** The policy gate hides the Settings row, persists `agentPane.yoloMode=false`, and reconciles every live provider session to native Yolo off. _(UT: stored-policy normalization and shared availability; E2E: `Feature.YoloMode`.)_
- [ ] `C293` `[new]` `[UT✓]` `[E2E]` **Settings explains Gemini automatic approval restrictions:** Settings keeps automatic approval visible and editable for Gemini and explains that automatic approval is restricted to trusted workspaces. _(PR #505; UT: provider notice matrix; zero-token E2E: `Feature.YoloMode`.)_
...
- [ ] `C318` `[new]` `[E2E]` **Nested prompt keeps resumable pane owner:** A nested agent's UUID-shaped `agent.prompt.submit` session ID does not replace the root session ID persisted for the pane; an explicit `agent.session.start` remains the ownership boundary.
...
- [ ] `C332` `[new]` `[E2E]` **Interactive sessions and connections expose local adoption sources:** Real keyboard interaction and shell creation emit the inherited Win32Host and App events with typed package and shell identities; local capture does not establish backend retention. _(PR #990; opt-in E2E: `Feature.TelemetryFunnels`.)_
- [ ] `C333` `[new]` `[E2E]` **ACP session creation and completed ordinary prompts expose engagement:** A real ACP session and two ordinary prompts share a session identity and produce successful response-completion telemetry, excluding Autofix from conversation depth. _(PR #990; opt-in E2E: `Feature.TelemetryFunnels`.)_
- [ ] `C334` `[new]` `[E2E]` **Concrete Autofix offers count once and only Run accepts:** Visible concrete repair cards emit one offer despite redraw; explicit Run emits a matching acceptance, while Insert and Reject do not. _(PR #990; opt-in E2E: `Feature.TelemetryFunnels`.)_
- [ ] `C335` `[new]` `[E2E]` **Visible foreground palette entry is distinct from hiding and submission:** Entering and reopening foreground prompt mode count separately from submission; closing, editing, and background mode do not create false entries. _(PR #990; opt-in E2E: `Feature.TelemetryFunnels`.)_
- [ ] `C336` `[new]` `[E2E]` **Provider changes expose bounded roles and transitions without unchanged reloads:** Accepted primary/delegate changes, including custom-to-custom, emit bounded role/from/to fields; an unchanged-provider reload emits none. _(PR #990; opt-in E2E: `Feature.TelemetryFunnels`.)_
- [ ] `C337` `[new]` `[E2E]` **Startup snapshots include inventory sidebar and no-agent states:** Startup telemetry includes selected and unused custom commands, zero inventories, both sidebar states, and launches without an agent session. _(PR #990; opt-in E2E: `Feature.TelemetryFunnels`.)_
- [ ] `C338` `[new]` `[E2E]` **Raw Autofix policy stays distinct from effective helper state:** Approved hot policy changes must first deliver native configuration with the matching raw policy and typed effective Boolean. Enabled policy reports a real forwarded shell failure as WTA ErrorDetected; blocked policy preserves rendered failure output but suppresses VT forwarding, helper error events, and Autofix prompts, without replaying prior failures. Hot refresh remains separately tracked in #991. _(PR #990; opt-in E2E: `Feature.TelemetryFunnels`.)_
- [ ] `C339` `[new]` `[E2E]` **Startup policy buckets retain configured inventory and effective provider gates:** Approved empty/allowlist and blocked/allowed policy variants preserve configured custom inventory while accurately reporting effective provider gates. _(PR #990; opt-in E2E: `Feature.TelemetryFunnels`.)_
- [ ] `C340` `[new]` `[E2E]` **Each same-process window emits one startup snapshot without activation duplicates:** Creating another window in the owned process emits its own AppCreated; activating existing windows does not emit extra startup snapshots. _(PR #990; opt-in E2E: `Feature.TelemetryFunnels`.)_
- [ ] `C341` `[new]` `[E2E]` **Startup Autofix policy labels match effective helper state:** With approved policy set before each fresh host launch, enabled policy reports a real shell failure as typed WTA ErrorDetected (vt_sequence, enabled/true). Blocked policy suppresses VT forwarding and Autofix prompts; a controlled nonzero shell exit with the helper retained reports connection_state, disabled/false, without an offer. This does not validate blocked shell-failure forwarding or unresolved policy hot refresh. _(PR #990; opt-in E2E: `Feature.TelemetryFunnels`.)_
