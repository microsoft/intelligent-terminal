...
## Why doesn't Agent Management show my session on the first tab right after I install?
...
## Why is the first-run experience (FRE) taking so long, or failing?
...
## Why won't Intelligent Terminal install on my Windows 10 machine?

The package manifest sets `MinVersion="10.0.22621.6060"` (Windows 11 22H2), so the MSIX install is blocked on earlier OS builds. This release is Windows 11 only.

**What to do:** Use Windows 11 (22H2 or later). Windows 10 support is planned for a later release.

## I installed a new agent CLI after the FRE — why isn't it tracked in Agent Management?
...
## Why doesn't Agent Management show my delegate-agent sessions?
...
**What to do:** Switch to the delegate's tab directly (or, for background `?<prompt>` work, watch the bottom-bar agent status indicator) to monitor its progress. Unified tracking of both agent-pane and delegate sessions is planned for a later release.
