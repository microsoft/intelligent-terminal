...
未来我们会添加一个新的provider：opencode。
我已经把opencode整个repo的内容添加到当前workspace了。

请先阅读文档以及相关内容(C:\ado\opencode\README.md)，然后下载或build opencode的acp验证一下拿到他的usage/cost数据的可行性，保存他的真实的acp输出结果。

再用test driven development workflow完成POC的开发。

opencode在main branch上的引入还在开发中（组里的其他member is working on that）。Here，我们当前只 investigate usages and costs 这一部分。

由于目前的main上还没有 opencode这一provider，
我先切换出一个branch来investigate usage / costs for opencode：

user/DinahK-2SO/acp-price-calc__investigate_opencode

你可以在这个branch上保存我们为 opencode开发的test-driven development 每一步的feature相关的代码，并把它推到远程仓库。

未来等main branch上的opencode provider reader之后，我们再整理这些功能代码到一个统一的 PR branch。
