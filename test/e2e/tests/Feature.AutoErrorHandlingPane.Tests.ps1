...
Describe 'Feature: Auto error handling card render + reject + AI correctness' -Tag 'Feature' -Skip:(-not $script:Ready) {
    ...
    It 'Visible agent pane flow works (recommendation card renders)' {
...
Describe 'Feature: Auto error handling target pane routing' -Tag 'Feature' -Skip:(-not $script:Ready) {
    ...
    It 'Auto error handling target pane is correct (Insert returns to the failing split pane)' {
        ...
        try {
            Start-Sleep -Milliseconds 400
            Invoke-FailingCommand -App $script:app -SessionId $script:targetShell.session_id -Command 'gti status' | Out-Null
            Wait-AutoErrorHandling -Listener $listener -TimeoutSec 45 | Out-Null
        } finally { Stop-WtEventListener -Listener $listener }
        ...
        Assert-Pane -App $script:app -SessionId $script:targetShell.session_id -Match 'git status' -TimeoutSec 12
        (Get-WtCapture -App $script:app -SessionId $script:otherShell.session_id -MaxLines 30) |
            Should -Not -Match 'git status' -Because 'Insert must not route the proposed fix to the non-failing pane'
        Send-WtKeys -App $script:app -SessionId $script:targetShell.session_id -Keys @('C-c')
    }
}
...
Describe 'Feature: Auto error handling Insert action' -Tag 'Feature' -Skip:(-not $script:Ready) {
    ...
    It 'Insert proposed fix works (types the fix into the shell pane)' {
        $sid = (Get-ActivePane -App $script:app).session_id
        $listener = Start-WtEventListener -App $script:app
        try {
            Start-Sleep -Milliseconds 400
...
Describe 'Feature: Auto error handling Run action' -Tag 'Feature' -Skip:(-not $script:Ready) {
    ...
    It 'Run proposed fix works (executes the fix in the shell pane)' {
        $sid = (Get-ActivePane -App $script:app).session_id
        $listener = Start-WtEventListener -App $script:app
        try {
            Start-Sleep -Milliseconds 400
...
Describe 'Feature: Auto error handling on stashed pane' -Tag 'Feature' -Skip:(-not $script:Ready) {
    ...
    It 'Stashed agent pane flow works (pre-warmed helper triggers while hidden)' {
        ...
        try {
            ...
            { Wait-AutoErrorHandling -Listener $listener -TimeoutSec 45 } | Should -Not -Throw
        } finally { Stop-WtEventListener -Listener $listener }
    }
}
...
Describe 'Feature: Auto error handling in a WSL pane (OSC 9001;ShellType end-to-end)' -Tag 'Feature' -Skip:(-not $script:WslReady) {
    BeforeAll {
        ...
        try {
            $script:wsl = New-WtTab -App $script:app -Command 'wsl.exe' -Title 'wsl-auto-error-handling'
    ...
    It 'Auto error handling proposes a Linux/bash fix (not PowerShell) in a WSL pane' {
        ...
        Assert-AI -Claim 'The fix command uses Linux/bash shell syntax (e.g. ls, grep, cat, forward-slash paths). It is NOT a Windows PowerShell command (no Get-ChildItem / Select-String / cmdlet-style Verb-Noun).' -Context $cardText
    }
}
