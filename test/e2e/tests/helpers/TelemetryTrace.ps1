function Start-TestTelemetryTrace {
    ...
    $process = Start-Process -FilePath (Get-Command pwsh).Source -Verb RunAs -PassThru `
        -ArgumentList "-NoProfile -File `"$collector`" -OutputDirectory `"$directoryPath`""
...
function Read-TestTelemetryTrace {
    ...
    foreach ($event in $events.SelectNodes("//*[local-name()='Event']")) {
        ...
        if ($guid -notin @('24a1622f-7da7-5c77-3303-d850bd1ab2ed', '4cfcff80-4e6b-5bfd-8ea1-d38e1226f70b', 'be579944-4d33-5202-e5d6-a7a57f1935cb')) { continue }
